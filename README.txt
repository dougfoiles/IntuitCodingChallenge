...
Problem #3
I wrote method GetJSONPath that read in the test json from a file. It parses the file 
and stores each individual "id" string into an arraylist. I then looped through the 
arraylist and checked if an individual "id" string contained the element passed to the 
method. If the element was a subitem extra parsing was done. I assumed I was not supposed
to use existing json libraries so I parsed the json myself. Optimized with knowing
there is only one level of sub items. 
